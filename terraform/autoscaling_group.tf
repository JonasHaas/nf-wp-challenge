# This Terraform module creates an Auto Scaling Group (ASG) with an Elastic Load Balancer (ELB) for a WordPress site, using a launch template and scaling policies to adjust the number of instances based on CPU utilization.
module "wp-asg" {
  ...
  user_data = base64encode(templatefile("${path.module}/wp-init.sh",
    {
      vars = {
